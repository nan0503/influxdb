...
func (t *floatWindowTable) advance() bool {
	...
	vals := t.mergeValues(stop.Int64Values())
	...
	if t.timeColumn != "" {
		...
		cr.cols[valueColIdx] = vals
		t.appendBounds(cr)
	} else {
		cr.cols[startColIdx] = start
		cr.cols[stopColIdx] = stop
		cr.cols[valueColIdxWithoutTime] = vals
	}
	t.appendTags(cr)
	return true
}
...
func (t *floatEmptyWindowSelectorTable) advance() bool {
	...
	vals := t.arrowBuilder()
	vals.Resize(storage.MaxPointsPerBlock)

	var cr *colReader

	switch t.timeColumn {
	case execute.DefaultStartColLabel:
		start := t.startTimes(vals)
		cr = t.allocateBuffer(start.Len())
		cr.cols[timeColIdx] = start
		t.appendBounds(cr)
	case execute.DefaultStopColLabel:
		stop := t.stopTimes(vals)
		cr = t.allocateBuffer(stop.Len())
		cr.cols[timeColIdx] = stop
		t.appendBounds(cr)
	default:
		start, stop, time := t.startStopTimes(vals)
	...
	cr.cols[valueColIdx] = vals.NewFloat64Array()
	t.appendTags(cr)
	return true
}
...
func (t *floatGroupTable) advance() bool {
	...
	if t.gc.Aggregate() == nil {
		...
		// because the references were retained, then we will
		// allocate a new buffer.
		colReader := t.allocateBuffer(length)
...
func (t *integerEmptyWindowSelectorTable) advance() bool {
	...
	cr.cols[valueColIdx] = vals.NewInt64Array()
	t.appendTags(cr)
	return true
}
...
func (t *unsignedEmptyWindowSelectorTable) advance() bool {
	...
	cr.cols[valueColIdx] = vals.NewUint64Array()
	t.appendTags(cr)
	return true
}
...
func (t *stringEmptyWindowSelectorTable) advance() bool {
	...
	cr.cols[valueColIdx] = vals.NewBinaryArray()
	t.appendTags(cr)
	return true
}
...
func (t *booleanEmptyWindowSelectorTable) advance() bool {
	...
	cr.cols[valueColIdx] = vals.NewBooleanArray()
	t.appendTags(cr)
	return true
}
